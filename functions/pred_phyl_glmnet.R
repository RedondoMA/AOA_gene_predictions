pred_phyl_glmnet=function(tree, traits, opt.thres=FALSE, opt.lam=FALSE,lam=0.05,prob=0.5, mc=TRUE){
...
cross_validation_opt=function(var1,tree,labels, opt.thres, prob, lam, opt.lam){#this function will be later run in lapply. var1 is each column of the trait table
  ...
  train_PEM_val<-PEM.forcedSimple(#I obtain the PEMs forcing the a parameter to 0
    y = data_val[,1],
    x = NULL,
    w = opt.pgraph,
    a = 0) 
